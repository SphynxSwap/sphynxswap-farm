...
contract SphynxToken is BEP20, Manageable {
    ...
    address payable public marketingWallet =
        payable(0x982687617bc9a76420138a0F82b2fC1B8B11BbE3);
    address payable public developmentWallet =
        payable(0x4A48062b88d5B8e9f0B7A5149F87288899C2d7f9);
    ...
    modifier onlyMasterChefAndBridge() {
        require(
            msg.sender == masterChef || msg.sender == sphynxBridge,
            "Permission Denied"
        );
        _;
    }
    ...
    event LotteryAddressUpdated(
        address indexed newLotteryAddress,
        address indexed oldLotteryAddress
    );
    ...
    function updateSphynxSwapRouter(address newAddress) public onlyManager {
        ...
        address _sphynxSwapPair = ISphynxFactory(sphynxSwapRouter.factory())
    ...
    function updateMasterChef(address _masterChef) public onlyManager {
        require(
            ...
            "SPHYNX: MasterChef already exists!"
        );
        masterChef = _masterChef;
    ...
    function updateLotteryAddress(address newLotteryAddress)
        public
        onlyManager
    {
        require(
            newLotteryAddress != lotteryAddress,
            "SPHYNX: The lottery wallet is already this address"
        );
        ...
        if (takeFee) {
            ...
            if (_isGetFees[from] || _isGetFees[to]) {
                if (block.number - blockNumber <= 5) {
                    ...
                    if (sendToLottery) {
                        uint256 lotteryAmount = amount.mul(lotteryFee).div(
                            10**2
                        );
