...
contract sSphynx is IsSPH, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgSPH public gSPH; // additional staked supply (governance token)
    ...
    function setgSPH(address _gSPH) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gSPH) == address(0), "gSPH:  gSPH already set");
        require(_gSPH != address(0), "gSPH:  gSPH is not a valid contract");
        gSPH = IgSPH(_gSPH);
    }
    ...
    /**
        @notice increases rSPH supply to increase staking balances relative to profit_
    ...
    function toG(uint256 amount) external view override returns (uint256) {
        return gSPH.balanceTo(amount);
    }

    function fromG(uint256 amount) external view override returns (uint256) {
        return gSPH.balanceFrom(amount);
    }

    // Staking contract holds excess rSPH
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gSPH.balanceFrom(IERC20(address(gSPH)).totalSupply())).add(
