...
contract SphynxBondDepository is SphynxAccessControlled {
  ...
  function maxPayout(uint256 _BID) public view returns (uint256) {
    return SPH.totalSupply().mul(bonds[_BID].terms.maxPayout).div(100000);
  }
  ...
  /**
   * @notice calculate current ratio of debt to SPH supply
   * @param _BID uint
